...
namespace GRUT {
	...
	private:
		int														m_id = -1;
		std::function<void(std::shared_ptr<Job> param)>	m_entryPoint = nullptr;
		uintptr_t												m_param;
	...
	public:
		explicit Job(std::function<void(std::shared_ptr<Job> param)> p_entryPoint, int p_id) : m_entryPoint(p_entryPoint), m_id(p_id) {}
		explicit Job(std::function<void(std::shared_ptr<Job> param)> p_entryPoint, uintptr_t p_param, JobPriority p_priority) : m_entryPoint(p_entryPoint), m_param(p_param), m_priority(p_priority) {}
		~Job() = default;
		Job(Job &&old) :
