...
namespace GRUT {
  bool AABB::Contains(const Vector<3>& p_point) const {
    return 
  ...
  void AABB::Expand(const float amount) {
    m_size += amount * Vector<3>(1.0f, 1.0f, 1.0f);
    m_extents = 0.5 * m_size;
    m_min = m_center - m_extents;
    m_max = m_center + m_extents;
  }

  void AABB::Encapsulate(const Vector<3>& point) {
    m_min[0] = min(m_min.x(), point.x());
    m_min[1] = min(m_min.y(), point.y());
    m_min[2] = min(m_min.z(), point.z());
    m_max[0] = max(m_max.x(), point.x());
    m_max[1] = max(m_max.y(), point.y());
    m_max[2] = max(m_max.z(), point.z());
  }
  void AABB::Encapsulate(const AABB& other) {
    m_min[0] = min(m_min.x(), other.m_min.x());
    m_min[1] = min(m_min.y(), other.m_min.y());
    m_min[2] = min(m_min.z(), other.m_min.z());
    m_max[0] = max(m_max.x(), other.m_max.x());
    m_max[1] = max(m_max.y(), other.m_max.y());
    m_max[2] = max(m_max.z(), other.m_max.z());
  }
  AABB AABB::Encapsulate(const AABB& a, const AABB& b) {
    Vector<3> aabbMin = Vector<3>{
      min(a.m_min.x(), b.m_min.x()),
      min(a.m_min.y(), b.m_min.y()),
      min(a.m_min.z(), b.m_min.z())
    };
    Vector<3> aabbMax = Vector<3>{
