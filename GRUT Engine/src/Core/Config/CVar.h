...
namespace GRUT {
  ...
  private:
    std::string m_value;
  public:
    CVarString(const std::string& p_name, const std::string_view p_defaultValue) : ICVar(name), m_value(p_defaultValue) {}
    explicit CVarString(const std::string_view p_name) : ICVar(name) {}
    inline void SetVal(const std::string_view p_strVal) override { m_value = p_strVal; }
